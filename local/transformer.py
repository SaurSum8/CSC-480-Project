...
def main():
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4, weight_decay=0.01, eps=1e-9)

    # Training Data
    subset = dsRaw["train"].select(range(len(dsRaw["train"])))
    pairs = [(encode_src(ex["english"]), encode_tgt(ex["non_english"])) for ex in subset]
    ...
    # Validation Data
    valSet = dsRaw["dev"].select(range(len(dsRaw["dev"]) // 2))
    pairsVal = [(encode_src(ex["english"]), encode_tgt(ex["non_english"])) for ex in valSet]
    ...
    src_ids = encode_src("I am a very intelligent student.")
    ...
    # Test
    tSet = dsRaw["dev"].select(range(len(dsRaw["dev"]) // 2 + 1, len(dsRaw["dev"])))
    pairsT = [(encode_src(ex["english"]), encode_tgt(ex["non_english"])) for ex in tSet]
    ...
    # Save the model
    torch.save(model.state_dict(), "transformer_translation5.pt")
